...
function SkillHubAuthentication({ refreshAuth }: { refreshAuth: () => Promise<void> }) {
      ...
      <div className="skillhub-auth-flow" aria-label="连接步骤">
        ...
        <form className="skillhub-publish-form skillhub-auth-step" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <div className="skillhub-publish-card__actions">
            <button className="primary-button skillhub-auth-step__button" type="submit" disabled={isSubmitting || !token.trim()}>
...
export function PublishRoute() {
  return (
    <PublishingWorkbench
      ...
      renderAuthentication={(refreshAuth) => <SkillHubAuthentication refreshAuth={refreshAuth} />}
    />
  );
}
